...
func (s *Server) loadRecords(ctx context.Context) {
	rows, err := s.db.Query(ctx, "SELECT db_name, db_user, COALESCE(last_backup_at::timestamptz, 'epoch'::timestamptz), COALESCE(last_backup_key, ''), backup_count, s3_prefix FROM backup_tracker")
	if err != nil {
	...
	for rows.Next() {
		var r BackupRecord
		var t time.Time
		if err := rows.Scan(&r.DBName, &r.DBUser, &t, &r.LastBackupKey, &r.BackupCount, &r.S3Prefix); err != nil {
...
func (s *Server) upsertRecord(ctx context.Context, r *BackupRecord) error {
	...
	_, err := s.db.Exec(ctx, `INSERT INTO backup_tracker (db_name, db_user, last_backup_at, last_backup_key, backup_count, s3_prefix)
		VALUES ($1, $2, $3, $4, $5, $6)
...
func (s *Server) backupTarget(ctx context.Context, t backupTarget) error {
	...
	key := fmt.Sprintf("backups/%s/%s.sql.gz", t.Label, timestamp)
	...
	now := time.Now()
	prefix := fmt.Sprintf("backups/%s", t.Label)
	rec := &BackupRecord{
		DBName:        t.Label,
		DBUser:        t.User,
		LastBackupAt:  &now,
	...
	s.mu.RLock()
	existing, exists := s.records[t.Label]
	s.mu.RUnlock()
	if exists {
...
func (s *Server) handleTriggerBackup(w http.ResponseWriter, r *http.Request) {
	...
	for _, t := range targets {
		if t.Label == dbName || t.Name == dbName {
			target = &t
			break
		}
	}
